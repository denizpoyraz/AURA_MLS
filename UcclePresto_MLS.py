...
pathucclefiles = '/home/poyraden/Analysis/AURA_MLS/UccleData/'
...
for il in all_lines:
    ...
    tmp = pathucclefiles + tmp + '.csv'

    matcheddates.append(tmp)
print(matcheddates[0:10], len(matcheddates))

print('all files', len(all_lines), 'match files', len(matcheddates))
...
for filename in matcheddates:
    ...
    if ((len(xuccle) < 15) | (len(xuccle) == 0)):
        ...
        problem.write(str(df.at[df.first_valid_index(), 'Header_Date']) + '\n')
        continue
    ...
    header_date = df.at[df.first_valid_index(), 'Header_Date']
    ddate = [df.at[df.first_valid_index(), 'Header_Date']] * len(ymain)
    # im = 23
    im = 28
    ##ib = 8
    ib = 6
...
dfcp.to_csv("/home/poyraden/Analysis/AURA_MLS/New/MLS_UccleInterpolated_2004-2019_presto.csv")
